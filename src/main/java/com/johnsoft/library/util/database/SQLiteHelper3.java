 ...
 */
public abstract class SQLiteHelper3
{
	private static final String TAG = SQLiteHelper3.class.getSimpleName();
	...
	public SQLiteHelper3(Context context, String name, CursorFactory factory, int version)
	{
		if (version < 1)
		...
		if (mRDatabases == null)
		{
			...
			try
			{
				...
				mRDatabases = new Pool<SQLiteDatabase>(8)
				{
					...
					{
						synchronized (SQLiteHelper3.this)
						{
							if (mIsInitializing)
							{
								throw new IllegalStateException("getDatabase called recursively");
